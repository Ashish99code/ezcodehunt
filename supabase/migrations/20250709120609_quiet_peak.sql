/*
  ...
  1. Categories
    - Insert 8 tool categories with proper slugs and descriptions
  2. Sample Tools
    - Insert 5 sample tools with complete metadata
  3. Tool Features
    - Add features for each sample tool
  4. Tool Screenshots
    - Add sample screenshots for tools
  5. Admin Settings
...
DECLARE
  ...
  code_editor_id uuid;
  browser_ide_id uuid;
  code_generators_id uuid;
BEGIN
  -- Get category IDs
  SELECT id INTO ai_assistant_id FROM categories WHERE slug = 'ai-assistant';
  SELECT id INTO code_editor_id FROM categories WHERE slug = 'code-editor';
  SELECT id INTO browser_ide_id FROM categories WHERE slug = 'browser-ide';
  SELECT id INTO code_generators_id FROM categories WHERE slug = 'code-generators';
...
BEGIN
  ...
  SELECT id INTO tool_id FROM tools WHERE slug = 'github-copilot';
  IF tool_id IS NOT NULL THEN
    INSERT INTO tool_features (tool_id, feature_name, feature_description, category)
    SELECT tool_id, feature_name, feature_description, category
    FROM (VALUES
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM tool_features 
      WHERE tool_features.tool_id = tool_id 
      AND tool_features.feature_name = v.feature_name
    );
  END IF;

  -- Cursor IDE features
  SELECT id INTO tool_id FROM tools WHERE slug = 'cursor-ide';
  IF tool_id IS NOT NULL THEN
    INSERT INTO tool_features (tool_id, feature_name, feature_description, category)
    SELECT tool_id, feature_name, feature_description, category
    FROM (VALUES
  ...
  -- Tabnine features
  SELECT id INTO tool_id FROM tools WHERE slug = 'tabnine';
  IF tool_id IS NOT NULL THEN
    INSERT INTO tool_features (tool_id, feature_name, feature_description, category)
    SELECT tool_id, feature_name, feature_description, category
    FROM (VALUES
  ...
  -- Replit features
  SELECT id INTO tool_id FROM tools WHERE slug = 'replit';
  IF tool_id IS NOT NULL THEN
    INSERT INTO tool_features (tool_id, feature_name, feature_description, category)
    SELECT tool_id, feature_name, feature_description, category
    FROM (VALUES
  ...
  -- CodeT5 features
  SELECT id INTO tool_id FROM tools WHERE slug = 'codet5';
  IF tool_id IS NOT NULL THEN
    INSERT INTO tool_features (tool_id, feature_name, feature_description, category)
    SELECT tool_id, feature_name, feature_description, category
    FROM (VALUES
...
BEGIN
  ...
  IF tool_id IS NOT NULL THEN
    ...
    SELECT tool_id, image_url, caption, alt_text, sort_order, is_main
    FROM (VALUES
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM tool_screenshots 
      WHERE tool_screenshots.tool_id = tool_id 
      AND tool_screenshots.image_url = v.image_url
    );
  END IF;
...
WHERE NOT EXISTS (
  SELECT 1 FROM admin_settings WHERE admin_settings.key = v.key
);